...
**`cos` is built in Cowork on the Productivity plugin.** It's a local-file personal OS covering four domains — `finances`, `knowledge-base`, `learning-pipeline`, `tasks-calendar` — with four interaction patterns: an always-on dashboard (`dashboard.html` — Basecamp-style snapshot generated by `scripts/generate_dashboard.py`, opened via `cos dashboard` or daily schedule; old `cos-dashboard-refresh` retired per PRD-cos-dashboard-basecamp.md), a scheduled Daily Digest, on-demand skills (`/today`, `/research`, `/capture`), and a sign-off-gated Autonomous Builder. All state lives in plain local files under `~/cos/`. Connectors (Calendar, GitHub, web) and external systems (Hermes, the Obsidian vault) are **sources** workflows read from — never storage. The HTML snapshot dashboard (via generator + `cos dashboard`) is primary; the Textual TUI is a live companion (keyboard/power-user view over the same data layer). On macOS, dashboard actions use the `cos://` URL handler (Ghostty smart focus, no background processes; one-time: `cos setup-macos-handler`). Build order and final flow per `PRD-cos-dashboard-basecamp.md` (sign-off phase complete) and `PRD-cos.md`.
...
- **Textual TUI** — Python terminal-app framework; live companion/keyboard view for `cos` (HTML snapshot dashboard is primary via generator + `cos dashboard`); reads same data layer + PRD-cos-dashboard-basecamp.md sign-off flow
